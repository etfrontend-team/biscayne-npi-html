...
export default function initToursFilter() {
  ...
  const cards = Array.from(grid.querySelectorAll('.adventure-card'))
  ...
  const getFilteredCards = () => cards.filter((card) => currentFilter === 'all' || card.dataset.location === currentFilter)
  ...
  const render = () => {
    ...
    cards.forEach((card) => {
      const matches = currentFilter === 'all' || card.dataset.location === currentFilter
